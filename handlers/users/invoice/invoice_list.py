...
from handlers.users.invoice.utils import generate_invoice_list_body
...
@dp.message_handler(Command("list_invoices"))
async def list_invoices_handler(message: Message):
    page = 1
    ...
    contracts = Contract.select().join(UserToContractConnector).where(UserToContractConnector.user_id == user.id,
                                                                      Contract.type == 1,
                                                                      UserToContractConnector.is_hidden == False) \
    ...
    invoice_list_body = invoice_list_body if invoice_list_body else lang.ru["invoice_no_active_invoices"]
    total_pages = math.ceil(len(contracts) / 3)

    page_contents = lang.ru["invoice_active_list"] + invoice_list_body
